...
void WorldSession::SendGMTicket(const GMTicket& ticket, time_t now/* = time(nullptr)*/) const
{
    ...
    data << uint32(ticket.GetId());             // WotLK: Ticket id
...
void WorldSession::HandleGMTicketCreateOpcode(WorldPacket& recv_data)
{
    ...
    recv_data.read_skip<uint8>();                           // WotLK: unknown bool

    uint32 chatDataLineCount;
    recv_data >> chatDataLineCount;

    for (uint32 i = 0; i < chatDataLineCount; i++)
        recv_data.read_skip<uint32>();                      // WotLK: chat timestamps
...
void WorldSession::HandleGMSurveySubmitOpcode(WorldPacket& recv_data)
{
    ...
    // Check if all provided question ids match the survey entry
    {
        for (auto itr = answers.begin(); itr != answers.end(); ++itr)
        {
            ...
            // WotLK: Check if provided answer is allowed for this question
            bool allowed = false;

            for (uint32 k = 0; k < sGMSurveyAnswersStore.GetNumRows(); ++k)
            {
                if (GMSurveyAnswersEntry const* answer = sGMSurveyAnswersStore.LookupEntry(k))
                    allowed = (answer->questionID == itr->first && answer->sortIndex == itr->second);
            }
...
void WorldSession::SendGMTicketResponseError() const
{
    WorldPacket data(SMSG_GMTICKET_RESPONSE_ERROR);
    SendPacket(data);
}

void WorldSession::SendGMTicketResponseResolve(bool survey) const
{
...
void WorldSession::SendGMTicketResponse(GMTicket* ticket) const
{
    std::string response = ticket->GetConclusion();
    utf8limit(response, 3999);                              // 3999 bytes limit for a response

    WorldPacket data(SMSG_GMTICKET_GET_RESPONSE, 4);
    data << uint32(1);                                      // Response id
    data << uint32(ticket->GetId());                        // Ticket id
    data << ticket->GetText();                              // Ticket text: data, should never exceed 1999 bytes
    data << response;                                       // Response text 1: data, should never exceed 3999 bytes
    data << uint8(0);                                       // Response text 2: empty string
    data << uint8(0);                                       // Response text 3: empty string
    data << uint8(0);                                       // Response text 4: empty string
    SendPacket(data);
}
...
void WorldSession::HandleGMResponseResolveOpcode(WorldPacket& /*recv_data*/)
{
    if (sTicketMgr.GetSystemStatus() != GMTICKET_QUEUE_STATUS_ENABLED)
    {
        SendGMTicketResponseError();
        return;
    }

    if (sTicketMgr.Resolve(sTicketMgr.GetTicketByPlayer(GetPlayer()->GetObjectGuid()), "CMSG_GMTICKET_RESOLVE_RESPONSE", this))
        SendGMTicketResponseResolve(false);
    else
        SendGMTicketResponseError();
}
