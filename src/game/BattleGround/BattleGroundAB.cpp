...
void BattleGroundAB::HandlePlayerClickedOnFlag(Player* player, GameObject* go)
{
    ...
    else if (m_nodeStatus[node] == ABNodeStatus(BG_AB_NODE_STATUS_ALLY_CONTESTED) || m_nodeStatus[node] == ABNodeStatus(BG_AB_NODE_STATUS_HORDE_CONTESTED))
    {
        ...
        else
        {
            ...
            // update achievements
            player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BG_OBJECTIVE_CAPTURE, 123);
...
// Function that returns true if all nodes are controlled by the given team
bool BattleGroundAB::AreAllNodesControlledByTeam(Team team)
{
    for (unsigned char m_Node : m_nodeStatus)
