...
BattleGroundAV::BattleGroundAV(): m_mineYellTimer(BG_AV_MINE_YELL), m_honorMapComplete(0), m_repTowerDestruction(0), m_repCaptain(0), m_repBoss(0), m_repOwnedGrave(0), m_repOwnedMine(0), m_repSurviveCaptain(0), m_repSurviveTower(0), m_spiritHealersSet(false)
{
...
void BattleGroundAV::AddPlayer(Player* player)
{
    ...
    // spawn initial spirit healers
    if (!m_spiritHealersSet)
    {
        for (uint8 i = 0; i < BG_AV_MAX_GRAVEYARDS; ++i)
        {
            uint32 entry = 0;
            if (avNodeDefaults[i].initialOwner != TEAM_INDEX_NEUTRAL)
                entry = avNodeDefaults[i].initialOwner == TEAM_INDEX_ALLIANCE ? BG_NPC_SPIRIT_GUIDE_ALLIANCE : BG_NPC_SPIRIT_GUIDE_HORDE;

            // spawn only the healers that have a default
            if (entry)
            {
                if (Creature* healer = player->SummonCreature(entry, avGraveyardData[i].x, avGraveyardData[i].y, avGraveyardData[i].z, avGraveyardData[i].o, TEMPSPAWN_DEAD_DESPAWN, 0))
                    m_nodes[i].spiritHealerGuid = healer->GetObjectGuid();
            }
        }

        m_spiritHealersSet = true;
    }
}
...
void BattleGroundAV::ProcessPlayerDestroyedPoint(AVNodeIds node)
{
    ...
    else
    {
        ...
        Creature* herald = GetSingleCreatureFromStorage(ownerTeamIdx == TEAM_INDEX_ALLIANCE ? BG_AV_NPC_STORMPIKE_HERALD : BG_AV_NPC_FROSTWOLF_HERALD);
        if (!herald)
        {
            sLog.outError("BattleGroundAV: Cannot find battleground herald.");
            return;
        }

        uint32 healerEntry = ownerTeamIdx == TEAM_INDEX_ALLIANCE ? BG_NPC_SPIRIT_GUIDE_ALLIANCE : BG_NPC_SPIRIT_GUIDE_HORDE;
        if (Creature* healer = herald->SummonCreature(healerEntry, avGraveyardData[node].x, avGraveyardData[node].y, avGraveyardData[node].z, avGraveyardData[node].o, TEMPSPAWN_DEAD_DESPAWN, 0))
            m_nodes[node].spiritHealerGuid = healer->GetObjectGuid();
    }
}
...
void BattleGroundAV::ProcessPlayerDefendsPoint(Player* player, AVNodeIds node)
{
    ...
    // update achievements
    player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BG_OBJECTIVE_CAPTURE, objId);
...
void BattleGroundAV::ProcessPlayerAssaultsPoint(Player* player, AVNodeIds node)
{
    ...
    if (!isTower)
    {
        ...
        // despawn spirit healer
        if (Creature* healer = GetBgMap()->GetCreature(m_nodes[node].spiritHealerGuid))
...
bool BattleGroundAV::CheckAchievementCriteriaMeet(uint32 criteriaId, Player const* source, Unit const* target, uint32 miscvalue1)
{
    switch (criteriaId)
    {
        case BG_AV_ACHIEV_CRIT_ALL_COUNTS_A1:
        case BG_AV_ACHIEV_CRIT_ALL_COUNTS_A2:
            return m_mineOwner[BG_AV_IRONDEEP_MINE_ID] == TEAM_INDEX_ALLIANCE && m_mineOwner[BG_AV_COLDTOOTH_MINE_ID] == TEAM_INDEX_ALLIANCE;
        ...
        case BG_AV_ACHIEV_CRIT_STORMPIKE_PERF4:
        case BG_AV_ACHIEV_CRIT_STORMPIKE_PERF5:
        case BG_AV_ACHIEV_CRIT_STORMPIKE_PERF6:
        case BG_AV_ACHIEV_CRIT_STORMPIKE_PERF7:
        case BG_AV_ACHIEV_CRIT_STORMPIKE_PERF8:
        case BG_AV_ACHIEV_CRIT_STORMPIKE_PERF9:
        case BG_AV_ACHIEV_CRIT_STORMPIKE_PERF10:
            return source->GetTeam() == ALLIANCE && m_homeTowersControlled[TEAM_INDEX_ALLIANCE] == BG_AV_MAX_TOWERS_PER_TEAM && m_enemyTowersDestroyed[TEAM_INDEX_ALLIANCE] == BG_AV_MAX_TOWERS_PER_TEAM;
        case BG_AV_ACHIEV_CRIT_FROSTWOLF_PERF1:
        case BG_AV_ACHIEV_CRIT_FROSTWOLF_PERF2:
        case BG_AV_ACHIEV_CRIT_FROSTWOLF_PERF3:
        case BG_AV_ACHIEV_CRIT_FROSTWOLF_PERF4:
        case BG_AV_ACHIEV_CRIT_FROSTWOLF_PERF5:
        case BG_AV_ACHIEV_CRIT_FROSTWOLF_PERF6:
        case BG_AV_ACHIEV_CRIT_FROSTWOLF_PERF7:
        case BG_AV_ACHIEV_CRIT_FROSTWOLF_PERF8:
        case BG_AV_ACHIEV_CRIT_FROSTWOLF_PERF9:
        case BG_AV_ACHIEV_CRIT_FROSTWOLF_PERF10:
            return source->GetTeam() == HORDE && m_homeTowersControlled[TEAM_INDEX_HORDE] == BG_AV_MAX_TOWERS_PER_TEAM && m_enemyTowersDestroyed[TEAM_INDEX_HORDE] == BG_AV_MAX_TOWERS_PER_TEAM;
    }
