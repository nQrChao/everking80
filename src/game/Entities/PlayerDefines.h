...
struct SpellModifier
{
    ...
    SpellModifier(SpellModOp _op, SpellModType _type, int32 _value, uint32 _spellId, uint64 _mask, uint32 _mask2, int32 _priority = 0, int16 _charges = 0)
        : op(_op), type(_type), charges(_charges), value(_value), mask(_mask, _mask2), spellId(_spellId), priority(_priority), isFinite(_charges>0), modId(GetModId())
    {}
