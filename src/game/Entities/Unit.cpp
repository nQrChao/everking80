...
uint32 Unit::DealDamage(Unit* dealer, Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const* spellProto, bool durabilityLoss, Spell* spell)
{
    ...
    if (dealer)
    {
        ...
        for (auto vShareDamageAura : vShareDamageAuras)
        {
            ...
            if (Unit* shareTarget = vShareDamageAura->GetCaster())
            {
                if (shareTarget != victim && (vShareDamageAura->GetMiscValue() & damageSchoolMask))
                {
                    SpellEntry const* shareSpell = vShareDamageAura->GetSpellProto();
                    uint32 shareDamage = uint32(damage * vShareDamageAura->GetModifier()->m_amount / 100.0f);
                    Unit::DealDamageMods(dealer, shareTarget, shareDamage, nullptr, damagetype);
                    Unit::DealDamage(dealer, shareTarget, shareDamage, nullptr, damagetype, GetSpellSchoolMask(shareSpell), shareSpell, false);
                }
            }
        }
    ...
    if (victim->IsPlayer())          // Killed player
    {
        ...
        if (killer)
        {
            if (killer->GetTypeId() == TYPEID_UNIT)
                playerVictim->GetAchievementMgr().UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_KILLED_BY_CREATURE, killer->GetEntry());
            else if (killer->GetTypeId() == TYPEID_PLAYER && victim != killer)
...
void Unit::HandleDamageDealt(Unit* dealer, Unit* victim, uint32& damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const* spellProto, bool duel_hasEnded)
{
    ...
    if (dealer)
        dealer->InterruptOrDelaySpell(victim, damagetype, spellProto);
...
void Unit::CalculateSpellDamage(SpellNonMeleeDamage* spellDamageInfo, int32 damage, SpellEntry const* spellInfo, WeaponAttackType attackType)
{
    ...
    if (damage > 0)
    {
        ...
        if (damageSchoolMask & SPELL_SCHOOL_MASK_NORMAL)
        {
            ...
            damage = damage - armor_affected_damage + CalcArmorReducedDamage(this, pVictim, armor_affected_damage);
        }
    }
    else
        damage = 0;
...
uint32 Unit::CalcNotIgnoreAbsorbDamage(uint32 damage, SpellSchoolMask damageSchoolMask, SpellEntry const* spellInfo /*= nullptr*/) const
{
    ...
    if (spellInfo)
    {
        ...
        for (auto citr : ignoreAbsorbForSpell)
...
float Unit::CalcArmorReducedDamage(WorldObject* attacker, Unit* victim, const float damage)
{
    ...
    {
        float maxArmorPen = 400 + 85 * victim->GetLevel();
        if (attacker->GetLevel() > 59)
            maxArmorPen += 4.5f * 85 * (victim->GetLevel() - 59);
        ...
        float armorPenPct = std::min(100.f, static_cast<Player*>(attacker)->GetArmorPenetrationPct());
...
void Unit::CalculateDamageAbsorbAndResist(Unit* caster, SpellSchoolMask schoolMask, DamageEffectType damagetype, const uint32 damage, uint32* absorb, int32* resist, bool canReflect, bool canResist, bool binary)
{
    ...
    for (AuraList::const_iterator i = vAbsorb.begin(); i != vAbsorb.end() && RemainingDamage > 0; ++i)
    {
        ...
        if (i_spellProto->SpellFamilyName == SPELLFAMILY_MAGE && i_spellProto->SpellFamilyFlags & uint64(0x0000000000000108))
        {
            ...
            for (auto aura : auras)
            {
                SpellEntry const* itr_spellProto = aura->GetSpellProto();
        ...
        switch (spellProto->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_DEATHKNIGHT:
            {
                ...
                if (spellProto->Id == 50461)
                {
                    ...
                    Unit::DealDamage(caster, auraCaster, ab_damage, nullptr, damagetype, schoolMask, nullptr, false);
                    continue;
                }
                break;
            }
    ...
    if (incanterAbsorption)
    {
        ...
        for (auto aura : auras)
        {
            ...
            {
                int32 amount = int32(incanterAbsorption * aura->GetModifier()->m_amount / 100);
...
void Unit::CalculateAbsorbResistBlock(Unit* caster, SpellNonMeleeDamage* spellDamageInfo, SpellEntry const* spellProto, WeaponAttackType attType)
{
    ...
    CalculateDamageAbsorbAndResist(caster, spellDamageInfo->schoolMask, SPELL_DIRECT_DAMAGE, absorb_affected_damage, &spellDamageInfo->absorb, &spellDamageInfo->resist, IsReflectableSpell(spellProto), IsResistableSpell(spellProto) && !spellProto->HasAttribute(SPELL_ATTR_EX5_NO_PARTIAL_RESISTS), IsBinarySpell(*spellProto));
...
float Unit::GetCritMultiplier(SpellSchoolMask dmgSchoolMask, uint32 /*creatureTypeMask*/, const SpellEntry* spell, bool heal) const
{
    ...
    if (spell && IsSpellAppliesAura(spell))
    {
        ...
        for (auto aura : auras)
        {
            if (aura->isAffectedOnSpell(spell))
                bonus += aura->GetModifier()->m_amount;
        }
        if (bonus)
...
float Unit::GetMissChance(const SpellEntry* entry, SpellSchoolMask schoolMask) const
{
    ...
    // AoE avoidance
    if (IsAreaOfEffectSpell(entry) || entry->HasAttribute(SPELL_ATTR_EX5_TREAT_AS_AREA_EFFECT))
...
void Unit::SetCurrentCastedSpell(Spell* newSpell)
{
    ...
    switch (CSpellType)
    {
        case CURRENT_GENERIC_SPELL:
        {
            ...
            {
                bool isChanneling = channeledSpell->getState() == SPELL_STATE_CHANNELING;
                if (!isChanneling || !channeledSpell->m_spellInfo->HasAttribute(SPELL_ATTR_EX5_ALLOW_ACTIONS_DURING_CHANNEL))
...
bool Unit::IsNonMeleeSpellCasted(bool withDelayed, bool skipChanneled, bool skipAutorepeat, bool forMovement, bool forAutoIgnore, bool forEquip) const
{
    ...
    if (Spell const* genericSpell = m_currentSpells[CURRENT_GENERIC_SPELL])
    {
        if (genericSpell->getState() != SPELL_STATE_FINISHED)
        {
            ...
            if (forEquip)
                specialResult = !genericSpell->m_spellInfo->HasAttribute(SPELL_ATTR_EX6_ALLOW_EQUIP_WHILE_CASTING);
    ...
    if (!skipChanneled)
    {
        if (Spell const* channeledSpell = m_currentSpells[CURRENT_CHANNELED_SPELL])
        {
            bool isChannelling = channeledSpell->getState() == SPELL_STATE_CHANNELING;
            bool isCasting = channeledSpell->getState() == SPELL_STATE_CASTING;
            bool attributeResult = false;
            if (isChannelling)
                attributeResult |= channeledSpell->m_spellInfo->HasAttribute(SPELL_ATTR_EX5_ALLOW_ACTIONS_DURING_CHANNEL);
            if (!forMovement)
                attributeResult |= channeledSpell->m_spellInfo->HasAttribute(SPELL_ATTR_EX4_ALLOW_CAST_WHILE_CASTING);
            if (forEquip && isCasting)
                attributeResult |= channeledSpell->m_spellInfo->HasAttribute(SPELL_ATTR_EX6_ALLOW_EQUIP_WHILE_CASTING);
    ...
    for (SpellAuraHolderMap::iterator iter = m_spellAuraHolders.begin(); iter != m_spellAuraHolders.end();)
    {
        if ((!iter->second->GetSpellProto()->HasAttribute(SPELL_ATTR_EX4_ALLOW_ENTERING_ARENA) &&
                ...
                (!iter->second->GetSpellProto()->HasAttribute(SPELL_ATTR_NO_IMMUNITIES) ||
                 !iter->second->GetSpellProto()->HasAttribute(SPELL_ATTR_DO_NOT_LOG)) &&
                !HasAreaAuraEffect(iter->second->GetSpellProto()) &&
                // not unaffected by invulnerability auras or not having that unknown flag (that seemed the most probable)
                (iter->second->IsPositive() != onleave))   // remove positive buffs on enter, negative buffs on leave
                || iter->second->GetSpellProto()->HasAttribute(SPELL_ATTR_EX5_REMOVE_ENTERING_ARENA))
        {
            RemoveSpellAuraHolder(iter->second);
            iter = m_spellAuraHolders.begin();
        }
...
Aura* Unit::GetAura(AuraType type, SpellFamily family, uint64 familyFlag, uint32 familyFlag2, ObjectGuid casterGuid) const
{
    ...
    for (auto aura : auras)
        if (aura->GetSpellProto()->IsFitToFamily(family, familyFlag, familyFlag2) &&
...
Aura* Unit::GetTriggeredByClientAura(uint32 spellId) const
{
    ...
    for (auto aura : auras)
    {
        ...
        if (holder->GetCasterGuid() == GetObjectGuid() && holder->GetSpellProto()->EffectTriggerSpell[aura->GetEffIndex()] == spellId)
            return aura;
    }
...
{
    WorldPacket data(SMSG_SPELLNONMELEEDAMAGELOG, (8 + 8 + 4 + 4 + 4 + 1 + 4 + 4 + 1 + 1 + 4 + 4 + 1));
    data << log->target->GetPackGUID();
    data << log->attacker->GetPackGUID();
    data << uint32(log->SpellID);
    data << uint32(log->damage);                            // damage amount
    data << uint32(std::max(int64(0), (int64(log->damage) - log->target->GetHealth()))); // overkill
...
{
    SpellNonMeleeDamage log(attacker, target, spellID, damageSchoolMask);
    log.damage = damage;
...
Unit* Unit::SelectMagnetTarget(Unit* victim, Spell* spell)
{
    ...
    {
        if (spell && spell->m_spellInfo->HasAttribute(SPELL_ATTR_EX_NO_REDIRECTION))
            return nullptr;
 ...
 */
uint32 Unit::SpellDamageBonusDone(Unit* victim, SpellSchoolMask schoolMask, SpellEntry const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack)
{
    ...
    {
        if (Unit* owner = GetOwner())
            return owner->SpellDamageBonusDone(victim, schoolMask, spellProto, pdamage, damagetype);
    }
    ...
    switch (spellProto->SpellFamilyName)
    {
        case SPELLFAMILY_MAGE:
        {
            ...
            {
                if (victim->isFrozen() || IsIgnoreUnitState(spellProto, IGNORE_UNIT_TARGET_NON_FROZEN))
                {
 ...
 */
uint32 Unit::SpellDamageBonusTaken(Unit* caster, SpellSchoolMask schoolMask, SpellEntry const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack)
{
...
uint32 Unit::MeleeDamageBonusTaken(Unit* caster, uint32 pdamage, WeaponAttackType attType, SpellSchoolMask schoolMask, SpellEntry const* spellProto, DamageEffectType damagetype, uint32 stack, bool flat)
{
    ...
    TakenTotalMod *= GetTotalAuraMultiplierByMiscValueForMask(SPELL_AURA_MOD_MECHANIC_DAMAGE_TAKEN_PERCENT, mechanicMask);
...
bool Unit::IsVisibleForOrDetect(Unit const* u, WorldObject const* viewPoint, bool detect, bool /*inVisibleList*/, bool is3dDistance, bool spell, bool ignorePhase) const
{
    ...
    if (!u->HasAuraType(SPELL_AURA_DETECT_STEALTH))
    {
        ...
        return true; // Shadow Sight seems to allow ignoring LoS check (not confirmed 100%)
...
void Unit::AddThreat(Unit* pVictim, float threat /*= 0.0f*/, bool crit /*= false*/, SpellSchoolMask schoolMask /*= SPELL_SCHOOL_MASK_NONE*/, SpellEntry const* threatSpell /*= nullptr*/ /*= nullptr*/)
{
    ...
    if (CanHaveThreatList())
        getThreatManager().addThreat(pVictim, threat, crit, schoolMask, threatSpell, false);
}
...
uint32 Unit::GetCreatePowers(Powers power) const
{
    switch (power)
    {
        ...
        case POWER_ENERGY:      return POWER_ENERGY_DEFAULT; // wotlk fills energy
...
void CharmInfo::InitVehicleCreateSpells()
{
    ...
    for (uint32 x = 0; x < CREATURE_MAX_SPELLS; ++x)
    {
        if (IsPassiveSpell(spells[x]))
            ...
            AddSpellToActionBar(spells[x], ActiveStates(0x8 + x), x);
    }
}
...
bool Unit::IsIgnoreUnitState(SpellEntry const* spell, IgnoreUnitState ignoreState) const
{
    ...
    for (auto stateAura : stateAuras)
    {
        if (stateAura->GetModifier()->m_miscvalue == ignoreState)
        {
            // frozen state absent ignored for all spells
            if (ignoreState == IGNORE_UNIT_TARGET_NON_FROZEN)
                return true;

            if (stateAura->isAffectedOnSpell(spell))
                return true;
        }
    }
...
Player* Unit::GetNextRandomRaidMember(float radius, AuraType noAuraType)
{
    ...
    // reserve place for players because resizing vector every unit push is unefficient (vector is reallocated then)
    ...
    for (GroupReference* itr = group->GetFirstMember(); itr != nullptr; itr = itr->next())
        if (Player* target = itr->getSource())
        {
            ...
            if (target != this && IsWithinDistInMap(target, radius) && target->IsAlive() && CanAssist(target))
