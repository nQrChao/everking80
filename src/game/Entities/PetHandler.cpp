...
bool CheckPathToTarget(Unit* pet, Unit* target)
{
    ...
    for (size_t i = 1; i < path.size(); ++i)
    {
        ...
        if (!pet->GetMap()->IsInLineOfSight(dataPrevious.x, dataPrevious.y, dataPrevious.z + collisionHeight, data.x, data.y, data.z + collisionHeight, pet->GetPhaseMask(), true))
            return false;
    }
...
{
    pet->SendPetActionFeedback(FEEDBACK_NO_PATH);
}
