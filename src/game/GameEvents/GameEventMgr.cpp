...
void GameEventMgr::LoadFromDB()
{
...

    {
        ...
        do
        {
            ...
            if (gameEvent.holiday_id != HOLIDAY_NONE)
            {
                if (!sHolidaysStore.LookupEntry(gameEvent.holiday_id))
                {
                    sLog.outErrorDb("`game_event` game event id (%i) have nonexistent holiday id %u.", event_id, gameEvent.holiday_id);
                    gameEvent.holiday_id = HOLIDAY_NONE;
                }
            }
        ...
        if (gameEvent.holiday_id)
        {
            if (HolidaysEntry* holiday = const_cast<HolidaysEntry*>(sHolidaysStore.LookupEntry(gameEvent.holiday_id)))
            {
                switch (holiday->Id)
                {
                    ...
                    case HOLIDAY_LOVE_IS_IN_THE_AIR:
                    {
                        time_t today = time(nullptr);
                        for (uint32 i = 0; i < 5; ++i) // yearly so send next 5 years
                        {
