                ...
                switch (action.type)
                {
                        ...
                        */
                        ...
                        if (!sLog.HasLogFilter(LOG_FILTER_EVENT_AI_DEV) && spellInfo)
                        {
                            ...
                            if (action.cast.target == TARGET_T_ACTION_INVOKER &&
                                    (IsSpellHaveEffect(spellInfo, SPELL_EFFECT_QUEST_COMPLETE) || IsSpellHaveEffect(spellInfo, SPELL_EFFECT_CREATE_RANDOM_ITEM) || IsSpellHaveEffect(spellInfo, SPELL_EFFECT_DUMMY)
                                     || IsSpellHaveEffect(spellInfo, SPELL_EFFECT_KILL_CREDIT_PERSONAL) || IsSpellHaveEffect(spellInfo, SPELL_EFFECT_KILL_CREDIT_GROUP)))
                    ...
                    case ACTION_T_SET_INST_DATA:
                        if (!(temp.event_flags & EFLAG_DIFFICULTY_ALL))
                            sLog.outErrorEventAI("Event %u Action %u. Cannot set instance data without difficulty event flags.", eventId, j + 1);
