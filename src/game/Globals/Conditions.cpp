    ...
    switch (m_condition)
    {
        ...
        case CONDITION_AD_COMMISSION_AURA:
        {
            ...
            for (const auto& aura : auras)
                if ((aura.second->GetSpellProto()->HasAttribute(SPELL_ATTR_ALLOW_WHILE_MOUNTED) || aura.second->GetSpellProto()->HasAttribute(SPELL_ATTR_IS_ABILITY)) && aura.second->GetSpellProto()->SpellVisual[0] == 3580)
                    return true;
            return false;
        }
        ...
        case CONDITION_ACHIEVEMENT:
        {
            ...
            {
                sLog.outErrorDb("CONDITION_ACHIEVEMENT (entry %u) is used on non player target. Target is %s", m_entry, target->GetGuidStr().c_str());
                return false;
            }
            switch (m_value2)
            {
                case 0: return ((Player*)target)->GetAchievementMgr().HasAchievement(m_value1);
                case 1: return !((Player*)target)->GetAchievementMgr().HasAchievement(m_value1);
            }
            return false;
        }
        ...
        case CONDITION_XP_USER:
        {
            if (target->GetTypeId() != TYPEID_PLAYER)
            {
                sLog.outErrorDb("CONDITION_XP_USER (entry %u) is used on non player target. Target is %s", m_entry, target->GetGuidStr().c_str());
                return false;
            }
            switch (m_value1)
            {
                case 0: return target->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_XP_USER_DISABLED);
                case 1: return !target->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_XP_USER_DISABLED);
            }
            return false;
        }
