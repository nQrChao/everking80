...
void Usage(char* prg)
{
    printf(
        ...
        "Example: %s -f 0 -i \"/opt/game\"", prg, prg);
    exit(1);
}
...
uint32 ReadMapDBC()
{
    printf("Read Map.dbc file dir: %s\n", input_path);
    DBCFile dbc("./dbc/Map.dbc");
...
void ReadLiquidTypeTableDBC()
{
    ...
    DBCFile dbc("./dbc/LiquidType.dbc");
    if (!dbc.open())
    {
...
void ExtractMapsFromMpq(uint32 build)
{
    ...
    printf("Convert map files, output: %s\n", output_filename);
...
void ExtractDBCFiles(int locale, bool basicLocale)
{
    ...
    {
        printf("Extracted DBC file, src: %s\n", iter->c_str());
        string filename = path;
        filename += (iter->c_str() + strlen("./dbc/"));
...
{
    printf("Extracting camera files: %s\n", input_path);
    DBCFile camdbc("./dbc/CinematicCamera.dbc");

    printf("11111111111222:%s\n", camdbc.open());
    if (!camdbc.open())
    {
